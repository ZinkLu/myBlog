...
对于TOB软件来说，为了适配多中数据库（比如国产数据库），想要降低特异性，目前的方式只能是多次查询:
...
嵌套集合模型是一种将树形结构看做是一种嵌套结构：

![Untitled](Untitled.png)

一个嵌套的结构表示这个节点有多少子类。同时，通过给通过给某一个集合的 **左** 和 **右** 来进行编码，编码规则如下，每一个集合的 **左** 和 **右** 的区间永远包含其所有子类的区间

![Untitled](Untitled%201.png)

这种方式又被称为**预排序遍历树算法（modified preorder tree traversal algorithm）**，即在查询前就已经通过对树的遍历将其左右边界都算好了并存储起来:

![Untitled](Untitled%202.png)

这种数据结构的好处是能够不递归得一口气获取所有的子类，不过我们需要先准备一些数据
...
最后，使用 lft 进行排序，当 id 乱序时，能够获取排序好的树:
